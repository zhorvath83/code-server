FROM codercom/code-server:4.2.0 AS code-server
...
RUN curl -fsSL https://apt.releases.hashicorp.com/gpg | apt-key add - && \
    ...
    apt-get update -y && \
    apt-get install -y --no-install-recommends net-tools iputils-ping wget vim jq gnupg software-properties-common python3 python3-pip mc ca-certificates unzip bzr curl terraform && \
    apt-get clean && \
